...
public class WebSocket {
     ...
     * @param msg    消息
     */
// 获取指定的WebSocket连接映射，然后遍历映射中的每个连接，向每个连接发送消息。也就实现了向队伍内所有成员发送消息的功能。
...
//    检查用户ID和团队ID是否有效。
//    从会话中获取用户对象。
//    根据团队ID将WebSocket对象存储在全局的ROOMS映射中。
//    将会话添加到SESSIONS列表中，并将用户ID和会话存储在SESSION_POOL映射中。
//    发送所有在线用户的信息。
//    ROOMs映射可以方便对WebSocket对象进行组织和管理，使在多用户得场景下，能精准快速找到目标WebSocket对象，并进行消息发送和管理
    @OnOpen
    ...
    public void onMessage(String message, @PathParam("userId") String userId) {
        //检查消息是否是‘ping’，如果是就回复ppong
        if ("PING".equals(message)) {
            sendOneMessage(userId, "pong");
            return;
        }
        //把收到得消息用Gson处理，将消息转换为MessageRequest对象，
...
//        }
    ...
    public void sendAllMessage(String message) {
        for (Session session : SESSIONS) {
            try {
                if (session.isOpen()) {
                    synchronized (session) {//用synchronized来确保关键字在多线程环境下安全访问
